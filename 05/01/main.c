...
const char* fragmentShaderSource =
	...
	"	vec3 ambient = (light.color * attenuation + lightAmbient) * material.ambient * texCol / 1.5f;\n"
	...
	"	FragColor = vec4(vec3(texture(texture1, texCoord).x * (ambient+diffuse+specular)), 1.0f);\n"
	"}\0";
...
const char* windowTitle  = "Small Key";
...
int main(void) {
	...
	vec3 lightPos = {0.7f, 0.5f, 0.4f};
	
	vec3 lightColor = {1.2f, 1.2f, 1.2f};
	float lightConstant  = 1.0f;
	float lightLinear    = 0.35f;
	float lightQuadratic = 0.44f;
	
	
	const struct aiScene* scene = aiImportFile( "assets/Key/Small Key 2.obj",
	...
	glBufferData(GL_ARRAY_BUFFER, meshes[0].verticesCount*sizeof(*meshes[0].vertices), &meshes[0].vertices[0].position[0], GL_STATIC_DRAW);
	
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshes[0].indicesCount*sizeof(*meshes[0].indices), &meshes[0].indices[0], GL_STATIC_DRAW);
	
	// position attribute
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(*meshes[0].vertices), (void*)0);
	// normal attribute
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(*meshes[0].vertices), (void*)offsetof(struct Vertex, normal));
	// texture attribute
	glEnableVertexAttribArray(2);
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(*meshes[0].vertices), (void*)offsetof(struct Vertex, texCoords));
	...
	unsigned char *texData = stbi_load("assets/Key/Key_SS00.png", &texWidth, &texHeight, &nrChannels, 0);
	if(texData) {
	...
	float objRot = 0.0f;
	glm_vec3_rotate(cameraFront, -0.11f, cameraRight);
	glm_vec3_rotate(cameraUp, -0.11f, cameraRight);
	...
	while(!glfwWindowShouldClose(window)) {
		...
		if(deltaTime >= 1.0) {
			...
			glUniform1f(glGetUniformLocation(shaderProgram, "material.ambient"), 1.000000f);
			glUniform1f(glGetUniformLocation(shaderProgram, "material.diffuse"), 0.470588f);
			glUniform1f(glGetUniformLocation(shaderProgram, "material.specular"), 0.449020f*1.5f);
			glUniform1f(glGetUniformLocation(shaderProgram, "material.shininess"), 2.0f);
			glUniform3f(glGetUniformLocation(shaderProgram, "lightAmbient"), clearR/12.0f, clearG/12.0f, clearB/12.0f);
			...
			glBindVertexArray(VAO);
			glDrawElements(GL_TRIANGLES, meshes[0].indicesCount, GL_UNSIGNED_INT, 0);
			glBindVertexArray(0);
