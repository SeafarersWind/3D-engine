...
const char* fragmentShaderSource =
	...
	"	float avgObjectColor = (objectColor.x + objectColor.y + objectColor.z) / 3.0f;\n"
	...
	"	vec3 ambient = material.ambient * light.color * attenuation * objectColor;\n"
	
	"	float diff = max((dot(norm, lightDir)+0.4f), 0.0f);\n"
	"	vec3 diffuse = diff * material.diffuse * light.color * attenuation * objectColor;\n"
	
	"	float spec = pow(max(dot(viewDir, reflectDir), 0.0f), material.shininess);\n"
	"	vec3 specular = material.specular * spec * light.color * attenuation * (vec3(avgObjectColor*3.0f) + objectColor/2.0f);\n"
	
	"	FragColor = vec4(vec3(texture(texture1, texCoord) * vec4(objectColor, 1.0f) * vec4((ambient + diffuse + specular), 1.0f)), 1.0f);\n"
	"}\0";
...
const char* windowTitle  = "Firefly";
...
int main(void) {
	...
	vec3 lightPos = {2.2f, 0.5f, 0.6f};
	float lightTargetDistance = 12.0f;
	vec3 lightTarget = {-lightTargetDistance, 0.0f, 0.0f};
	vec3 lightTargetDirection = {0.0f, 0.0f, -1.0f};
	float lightTargetSpeedup;
	
	vec3 lightColor = {1.0f, 1.0f, 1.0f};
	float lightConstant  = 1.0f;
	float lightLinear    = 0.35f;
	float lightQuadratic = 0.44f;
	...
	int VBO, VAO, EBO, circleVBO, lightVAO;
	glGenBuffers(1, &VBO);
	glGenBuffers(1, &EBO);
...
//	glEnableVertexAttribArray(1);
	...
	unsigned char *texData = stbi_load("push1.png", &texWidth, &texHeight, &nrChannels, 0);
	if(texData) {
	...
	texData = stbi_load("push2.png", &texWidth, &texHeight, &nrChannels, 0);
	if(texData) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, texWidth, texHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, texData);
		glGenerateMipmap(GL_TEXTURE_2D);
	} else {
	...
	while(!glfwWindowShouldClose(window)) {
		...
		while(tickTock >= 1.0) {
			...
			// light target speed
			float lightTargetSpeed = (sin(timer*5.0f)+1.1f)/196.0f;
			lightTargetSpeedup -= (0.004);
			float lightTargetAccelerate = 0.004f * (13.0f - glm_vec3_distance(lightPos, lightTarget));
			if(lightTargetAccelerate < 0.0f) lightTargetAccelerate = 0.0f;
			lightTargetSpeedup += lightTargetAccelerate;
			if(lightTargetSpeedup < 0.0f) lightTargetSpeedup = 0.0f;
			if(lightTargetSpeedup > 0.5f) lightTargetSpeedup = 0.5f;
			lightTargetSpeed += lightTargetSpeedup;
			if(lightTargetSpeed > 0.1f) lightTargetSpeed = 0.1f;
			
			// light target movement
			vec3 lightTargetAxis;
			glm_vec3_cross(lightTarget, lightTargetDirection, lightTargetAxis);
			glm_vec3_rotate(lightTarget, lightTargetSpeed, lightTargetAxis);
			glm_vec3_rotate(lightTargetDirection, lightTargetSpeed, lightTargetAxis);
			glm_vec3_rotate(lightTargetDirection, cos(timer*4.87f)/64.0f, lightTarget);
			
			//printf("%f\n", glm_vec3_distance(lightTarget, lightPos));
			//glm_vec3_rotate(lightTarget, lightTargetSpeed, lightRotAxis);
			//glm_vec3_rotate(lightRotAxis, -0.003f+lightTargetSpeedup, lightRotAxisAxis);
			//lightTarget[0] *= 6.0f;
			//lightTarget[1] *= 6.0f;
			//lightTarget[2] *= 6.0f;
			//glm_vec3_rotate(lightRotAxisAxis, 0.0001f, lightRotAxisAxisAxis);
			
			// light move
			vec3 lightMove = {
				(lightTarget[0]-lightPos[0])*((sin(timer*12.0f)+1.1f)/720.0f),
				(lightTarget[1]-lightPos[1])*((sin(timer*11.9f+M_PI*0.6667)+1.1f)/720.0f),
				(lightTarget[2]-lightPos[2])*((sin(timer*12.1f+M_PI*1.2222)+1.1f)/720.0f)
			};
			
			// light avoid cube
			float lightCloseness = 2.0f - sqrt(lightPos[0]*lightPos[0]+lightPos[1]*lightPos[1]+lightPos[2]*lightPos[2]);
			if(lightCloseness > 0.0f && glm_vec3_angle(lightPos, lightMove) > M_PI/2) {
				float lightTurnAngle = -(M_PI/2 - glm_vec3_angle(lightPos, lightMove)) * (lightCloseness*lightCloseness);
				vec3 lightTurnAxis;
				glm_vec3_cross(lightMove, lightPos, lightTurnAxis);
				glm_vec3_rotate(lightMove, lightTurnAngle, lightTurnAxis);
			}
			
			// light move
			lightPos[0] += lightMove[0];
			lightPos[1] += lightMove[1];
			lightPos[2] += lightMove[2];
		...
		//render
		if(deltaTime >= 1.0) {
			glClearColor(clearR/12.0f, clearG/12.0f, clearB/12.0f, 1.0f);
			...
			glUniform1f(glGetUniformLocation(shaderProgram, "material.diffuse"), 1.7f);
			glUniform1f(glGetUniformLocation(shaderProgram, "material.specular"), 0.4f);
			...
			//model matrix
			mat4 modelMat;
			vec3 modelRot = {sin(timer*0.1f+2.0f), sin(timer*0.1f+4.0f), sin(timer*0.1f)};
			glm_rotate_make(modelMat, timer*0.4f, modelRot);
			...
			//glm_mat4_identity(lightModelMat);
